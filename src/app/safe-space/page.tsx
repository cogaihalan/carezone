...
export default function SafeSpacePage() {
  ...
  return (
    <div className={`min-h-screen transition-all duration-500 ${
      ...
      {rippleAnimation && (
        <div
          className={`theme-ripple ${rippleAnimation.isDark ? 'theme-ripple-dark' : 'theme-ripple-light'}`}
          style={{
            left: rippleAnimation.x - 20,
            top: rippleAnimation.y - 20,
            width: 40,
            height: 40,
          }}
        />
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="text-center">
          ...
          {/* Audio player */}
          <div className={`rounded-2xl shadow-lg p-8 mb-12 max-w-2xl mx-auto transition-all duration-500 ${
            isDarkMode ? 'bg-gray-800' : 'bg-white'
          }`}>
            <div className="text-center mb-6">
              <h2 className={`text-2xl font-semibold mb-2 transition-colors duration-500 ${
          ...
          {/* Rest message */}
          <div className={`rounded-2xl shadow-lg p-12 max-w-3xl mx-auto transition-all duration-500 ${
            isDarkMode ? 'bg-gray-800' : 'bg-white'
          }`}>
            <div className={`text-3xl font-semibold mb-6 transition-colors duration-500 ${
              isDarkMode ? 'text-white' : 'text-gray-800'
            }`}>
            ...
            <p className={`text-lg leading-relaxed transition-colors duration-500 ${
              ...
              Đây là không gian an toàn của bạn. Không có gì cần phải làm,<br />
          ...
          <div className="fixed inset-0 pointer-events-none overflow-hidden">
            <div className={`absolute top-20 left-10 w-4 h-4 rounded-full opacity-60 animate-pulse ${
              isDarkMode ? 'bg-purple-400' : 'bg-blue-300'
            }`}></div>
            <div className={`absolute top-40 right-20 w-6 h-6 rounded-full opacity-40 animate-pulse delay-1000 ${
              isDarkMode ? 'bg-purple-300' : 'bg-blue-200'
            }`}></div>
            <div className={`absolute bottom-32 left-20 w-3 h-3 rounded-full opacity-50 animate-pulse delay-2000 ${
              isDarkMode ? 'bg-purple-500' : 'bg-blue-400'
            }`}></div>
            <div className={`absolute bottom-20 right-10 w-5 h-5 rounded-full opacity-60 animate-pulse delay-500 ${
              isDarkMode ? 'bg-purple-400' : 'bg-blue-300'
            }`}></div>
          </div>
        </div>
